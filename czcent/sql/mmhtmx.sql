SELECT 
 ...
 R1.RECEIVABLE AS 卖方中介费, 
 SUM( R.RECEIVABLE +  R.RECEIVABLE ) /T.DEAL_PRICE  AS 收费比例,
...
NVL(zjfwf.zjfwf,0)+NVL(mfzjf.mfzjf,0)+NVL(pgftc.pgftc,0)+NVL(dkfwf.dkfwf,0)+NVL(qtfy.qtfy,0) AS 总结利,
NVL(dspgf.dspgf,0) as 代收评估费,
NVL(pgf_cw.pgf_cw,0) AS 评估费,
T.STATUS AS 合同状态
FROM   BIZ_MM_CONTRACT T
JOIN (SELECT MC1.ID,SU.USERNAME,SD.NAME FROM BIZ_MM_CONTRACT MC1 LEFT JOIN BIZ_MMFY FY ON MC1.FY_ID = FY.ID LEFT JOIN SYS_USER SU ON SU.ID = FY.DUTY_USER_ID LEFT JOIN SYS_DEPARTMENT SD ON SD.ID = SU.DEPT_ID  ) DUTY
 ON DUTY.ID = T.ID
...
GROUP BY D.NAMEALL, D.NAME, U.USERNAME, T.CONTRACT_CODE, T.HANDWRITING_CODE,  T.IF_LIMIT,  T.DEAL_DATE,  T.REVIEW_DATE,  MMFY.CODE,  DUTY.NAME,  DUTY.USERNAME,  T.ADDRESS,  MMFY.BUILD_AREA, 
 P.NAME,  PP.NAME, T.DEAL_PRICE, R.RECEIVABLE, R1.RECEIVABLE, zjfwf.zjfwf, mfzjf.mfzjf, pgftc.pgftc, dkfwf.dkfwf, qtfy.qtfy, dspgf.dspgf, pgf_cw.pgf_cw, T.STATUS