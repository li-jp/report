...
from
 ...
 on u.user_id = zlzjfwfxz.area_user_id
  --租赁卖方中介费新增
 left join
 (select zlfee.area_user_id,sum(zlfee.price) zlmfzjf from report_zl_fee_info zlfee  where zlfee.fee_name = '卖方中介费' and to_char(zlfee.review_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(zlfee.review_date,'yyyy-MM-dd') <= '${MONTH_END}' and to_char(zlfee.input_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(zlfee.input_date,'yyyy-MM-dd') <= '${MONTH_END}' group by zlfee.area_user_id) zlmfzjfxz
 on u.user_id = zlmfzjfxz.area_user_id
  --买卖中介服务费结转
 left join
