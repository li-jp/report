...
from 
 (select usr.* from report_user_dept_info usr where (usr.nameall like '%片区%' or usr.username ='不算个人业绩') and usr.deptname <>'金融权证' and ((usr.status = '离职' and (to_date('${MONTH_END}','yyyy-MM-dd')-usr.Dimission_Date) <=35) or usr.status = '在职')) u 
 --买卖中介服务费新增
 left join
 ...
 where u.deptname like '%${text}%'
  ${if(len(pro_name)==0,"",
   ...
   and (mmzjxz.mmzjf     != 0
	 or mmmfzjxz.mmmfzjf != 0
	 or mmdkxz.mmdkf     != 0
	 or mmpgxz.mmpgf     != 0
	 or mmqtxz.mmqt      != 0
	 or zlzjxz.zlzjf     != 0
	 or zlmfzjxz.zlmfzjf != 0
	 or zlqtxz.zlqt      != 0
 	 or mmzjjz.mmzjf     != 0
	 or mmmfzjjz.mmmfzjf != 0
	 or mmdkjz.mmdkf     != 0
	 or mmpgjz.mmpgf     != 0
	 or mmqtjz.mmqt      != 0
 	 or zlzjjz.zlzjf     != 0
	 or zlmfzjjz.zlmfzjf != 0
	 or zlqtjz.zlqt      != 0)
