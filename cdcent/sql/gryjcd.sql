...
from
 (select usr.* from report_user_dept_info usr where (usr.nameall like '%分公司%' or usr.username ='不算个人业绩') and usr.deptname <>'金融权证' and ((usr.status = '离职' and (to_date('${MONTH_END}','yyyy-MM-dd')-to_date(to_char(usr.Dimission_Date,'yyyy-MM-dd'),'yyyy-MM-dd') <=35)) or (usr.status = '在职' and to_char(usr.induction_time,'yyyy-MM-dd') <= '${MONTH_START}' ))) u
 --买卖中介服务费新增
 left join
