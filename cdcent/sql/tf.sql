select u.*,
mmzjxz.mmzjf as 买卖中介服务费新增,mmzjxz.contract_code as 买卖中介服务费新增合同编号, mmzjxz.handwriting_code as 买卖中介服务费新增手写编号, mmzjxz.deal_date as 买卖中介服务费新增成单时间, 
mmmfzjfxz.mfzjf as 买卖卖方中介费新增,mmmfzjfxz.contract_code as 买卖卖方中介费新增合同编号, mmmfzjfxz.handwriting_code as 买卖卖方中介费新增手写编号, mmmfzjfxz.deal_date as 买卖卖方中介费新增成单时间, 
...
mmdbghfxz.dbghf as 买卖代办过户费新增, mmdbghfxz.contract_code as 买卖代办过户费新增合同编号 , mmdbghfxz.handwriting_code as 买卖代办过户费新增手写编号, mmdbghfxz.deal_date as 买卖代办过户费新增成单时间, 
mmdzfwfxz.dzfwf as 买卖垫资服务费新增, mmdzfwfxz.contract_code as 买卖垫资服务费新增合同编号 , mmdzfwfxz.handwriting_code as 买卖垫资服务费新增手写编号, mmdzfwfxz.deal_date as 买卖垫资服务费新增成单时间, 
...
mmzjjz.mmzjf as 买卖中介服务费结转, mmzjjz.contract_code as 买卖中介服务费结转合同编号, mmzjjz.handwriting_code as 买卖中介服务费结转手写编号, mmzjjz.deal_date as 买卖中介服务费结转成单时间, 
mmmfzjfjz.mfzjf as 买卖卖方中介费结转,mmmfzjfjz.contract_code as 买卖卖方中介费结转合同编号, mmmfzjfjz.handwriting_code as 买卖卖方中介费结转手写编号, mmmfzjfjz.deal_date as 买卖卖方中介费结转成单时间, 
...
mmdbghfjz.dbghf as 买卖代办过户费结转,mmdbghfjz.contract_code as 买卖代办过户费结转合同编号, mmdbghfjz.handwriting_code as 买卖代办过户费结转手写编号, mmdbghfjz.deal_date as 买卖代办过户费结转成单时间, 
mmdzfwfjz.dzfwf as 买卖垫资服务费结转,mmdzfwfjz.contract_code as 买卖垫资服务费结转合同编号, mmdzfwfjz.handwriting_code as 买卖垫资服务费结转手写编号, mmdzfwfjz.deal_date as 买卖垫资服务费结转成单时间, 
...
from 
 ...
 (select mmfee.user_id, mmfee.price mmzjf, mmfee.contract_code, mmfee.handwriting_code, mmfee.deal_date from report_mm_fee_info mmfee  where mmfee.fee_name = '中介服务费' and to_char(mmfee.review_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(mmfee.review_date,'yyyy-MM-dd') <= '${MONTH_END}' and to_char(mmfee.input_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(mmfee.input_date,'yyyy-MM-dd') <= '${MONTH_END}' and mmfee.price <= 0 group by mmfee.user_id, mmfee.price , mmfee.contract_code, mmfee.handwriting_code, mmfee.deal_date) mmzjxz
  on u.user_id = mmzjxz.user_id
   --买卖卖方中介费新增
 left join
 (select mmfee.user_id, mmfee.price mfzjf, mmfee.contract_code, mmfee.handwriting_code, mmfee.deal_date from report_mm_fee_info mmfee  where mmfee.fee_name = '中介服务费' and to_char(mmfee.review_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(mmfee.review_date,'yyyy-MM-dd') <= '${MONTH_END}' and to_char(mmfee.input_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(mmfee.input_date,'yyyy-MM-dd') <= '${MONTH_END}' and mmfee.price <= 0 group by mmfee.user_id, mmfee.price , mmfee.contract_code, mmfee.handwriting_code, mmfee.deal_date) mmmfzjfxz
 ...
 --买卖评估费新增
 left join 
 (select mmfee.user_id, mmfee.price mmpgf, mmfee.contract_code, mmfee.handwriting_code, mmfee.deal_date from report_mm_fee_info mmfee  where mmfee.fee_name = '评估费收入' and to_char(mmfee.review_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(mmfee.review_date,'yyyy-MM-dd') <= '${MONTH_END}' and to_char(mmfee.input_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(mmfee.input_date,'yyyy-MM-dd') <= '${MONTH_END}' and mmfee.price <= 0 group by mmfee.user_id, mmfee.price , mmfee.contract_code, mmfee.handwriting_code, mmfee.deal_date) mmpgxz
 on u.user_id = mmpgxz.user_id
  --买卖代办过户费新增
 left join 
 (select mmfee.user_id, mmfee.price dbghf, mmfee.contract_code, mmfee.handwriting_code, mmfee.deal_date from report_mm_fee_info mmfee  where mmfee.fee_name = '评估费收入' and to_char(mmfee.review_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(mmfee.review_date,'yyyy-MM-dd') <= '${MONTH_END}' and to_char(mmfee.input_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(mmfee.input_date,'yyyy-MM-dd') <= '${MONTH_END}' and mmfee.price <= 0 group by mmfee.user_id, mmfee.price , mmfee.contract_code, mmfee.handwriting_code, mmfee.deal_date) mmdbghfxz
 on u.user_id = mmdbghfxz.user_id
   --买卖垫资服务费新增
 left join 
 (select mmfee.user_id, mmfee.price dzfwf, mmfee.contract_code, mmfee.handwriting_code, mmfee.deal_date from report_mm_fee_info mmfee  where mmfee.fee_name = '评估费收入' and to_char(mmfee.review_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(mmfee.review_date,'yyyy-MM-dd') <= '${MONTH_END}' and to_char(mmfee.input_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(mmfee.input_date,'yyyy-MM-dd') <= '${MONTH_END}' and mmfee.price <= 0 group by mmfee.user_id, mmfee.price , mmfee.contract_code, mmfee.handwriting_code, mmfee.deal_date) mmdzfwfxz
 ...
 left join
  ...
  --买卖卖方中介费结转
 left join
  (select mmfee.user_id, mmfee.price mfzjf, mmfee.contract_code, mmfee.handwriting_code, mmfee.deal_date from report_mm_fee_info mmfee  where mmfee.fee_name = '中介服务费' and to_char(mmfee.review_date,'yyyy-MM-dd') < '${MONTH_START}' and to_char(mmfee.input_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(mmfee.input_date,'yyyy-MM-dd') <= '${MONTH_END}' and mmfee.price <= 0 group by mmfee.user_id, mmfee.price , mmfee.contract_code, mmfee.handwriting_code, mmfee.deal_date) mmmfzjfjz
 ...
 on u.user_id = mmpgjz.user_id
  --买卖代办过户费结转
 left join 
 (select mmfee.user_id, mmfee.price dbghf, mmfee.contract_code, mmfee.handwriting_code, mmfee.deal_date from report_mm_fee_info mmfee  where mmfee.fee_name = '评估费收入' and to_char(mmfee.review_date,'yyyy-MM-dd') < '${MONTH_START}' and to_char(mmfee.input_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(mmfee.input_date,'yyyy-MM-dd') <= '${MONTH_END}' and mmfee.price <= 0 group by mmfee.user_id, mmfee.price , mmfee.contract_code, mmfee.handwriting_code, mmfee.deal_date) mmdbghfjz
 on u.user_id = mmdbghfjz.user_id
   --买卖垫资服务费结转
 left join 
 (select mmfee.user_id, mmfee.price dzfwf, mmfee.contract_code, mmfee.handwriting_code, mmfee.deal_date from report_mm_fee_info mmfee  where mmfee.fee_name = '评估费收入' and to_char(mmfee.review_date,'yyyy-MM-dd') < '${MONTH_START}' and to_char(mmfee.input_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(mmfee.input_date,'yyyy-MM-dd') <= '${MONTH_END}' and mmfee.price <= 0 group by mmfee.user_id, mmfee.price , mmfee.contract_code, mmfee.handwriting_code, mmfee.deal_date) mmdzfwfjz
