SELECT SUBSTR(D.NAMEALL,0,INSTR(D.NAMEALL,'-',1,1)-1) AS 片区,
 D.NAME AS 签约店,
 U.USERNAME AS 签约人,
 ...
 SUM(R.RECEIVABLE) /T.PRICE AS 收费比例,
 T.STATUS AS 合同状态
FROM   BIZ_ZL_CONTRACT T
JOIN (SELECT ZC.ID,SU.USERNAME,SD.NAME FROM BIZ_ZL_CONTRACT ZC JOIN BIZ_ZLFY FY ON ZC.FY_ID = FY.ID JOIN SYS_USER SU ON SU.ID = ZC.DUTY_USER_ID JOIN SYS_DEPARTMENT SD ON SD.ID = SU.DEPT_ID ) DUTY
 ON DUTY.ID = T.ID
...
GROUP BY D.NAMEALL, D.NAME, U.USERNAME, T.CONTRACT_CODE, T.HANDWRITING_CODE,  T.DEAL_DATE,  T.REVIEW_DATE,  ZLFY.CODE,  DUTY.NAME, DUTY.USERNAME, D.NAME,  U.USERNAME,  T.ADDRESS,  ZLFY.BUILD_AREA,  P.NAME,  PP.NAME,T.PRICE, T.PAY_TYPE, R.RECEIVABLE, zjfwf.zjfwf, T.PRICE,T.STATUS