...
mmdkxz.mmdkf as 买卖贷款服务费新增, mmdkxz.contract_code as 买卖贷款服务费新增合同编号, mmdkxz.handwriting_code as 买卖贷款服务费新增手写编号, mmdkxz.deal_date as 买卖贷款服务费新增成单时间, 
 ...
 dlfwfxz.dlfwf as 代理服务费新增, dlfwfxz.contract_code as 代理服务费新增合同编号, dlfwfxz.handwriting_code as 代理服务费新增手写编号, dlfwfxz.deal_date as 代理服务费新增成单时间,
 ...
 dlfwfjz.dlfwf as 代理服务费结转, dlfwfjz.contract_code as 代理服务费结转合同编号, dlfwfjz.handwriting_code as 代理服务费结转手写编号, dlfwfjz.deal_date as 代理服务费结转成单时间
from 
 ...
 --代理代理服务费新增
  left join 
 (select zlfee.user_id, zlfee.price dlfwf, zlfee.contract_code, zlfee.handwriting_code, zlfee.deal_date from report_zl_fee_info zlfee  where zlfee.fee_name = '中介服务费' and to_char(zlfee.review_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(zlfee.review_date,'yyyy-MM-dd') <= '${MONTH_END}' and to_char(zlfee.input_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(zlfee.input_date,'yyyy-MM-dd') <= '${MONTH_END}' and zlfee.price <= 0 group by zlfee.user_id, zlfee.price , zlfee.contract_code, zlfee.handwriting_code, zlfee.deal_date) dlfwfxz
 on u.user_id = dlfwfxz.user_id
  --买卖中介服务费结转
 left join
 ...
 --代理代理服务费新增
  left join 
 (select zlfee.user_id, zlfee.price dlfwf, zlfee.contract_code, zlfee.handwriting_code, zlfee.deal_date from report_zl_fee_info zlfee  where zlfee.fee_name = '中介服务费' and to_char(zlfee.review_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(zlfee.review_date,'yyyy-MM-dd') <= '${MONTH_END}' and to_char(zlfee.input_date,'yyyy-MM-dd') >= '${MONTH_START}' and to_char(zlfee.input_date,'yyyy-MM-dd') <= '${MONTH_END}' and zlfee.price <= 0 group by zlfee.user_id, zlfee.price , zlfee.contract_code, zlfee.handwriting_code, zlfee.deal_date) dlfwfjz
 on u.user_id = dlfwfjz.user_id
